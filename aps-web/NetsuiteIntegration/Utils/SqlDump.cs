...
{
    // Writes Encoding as its WebName; reading back is not supported.
    public sealed class EncodingJsonConverter : System.Text.Json.Serialization.JsonConverter<System.Text.Encoding>
    {
    ...
    public static class SqlDump
    {
        ...
        {
            Type type = typeof(T);
            PropertyInfo[] props = type.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(p => p.GetIndexParameters().Length == 0).ToArray();
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                Type colType = Nullable.GetUnderlyingType(pt) != null ? typeof(string) : u.IsEnum ? typeof(string) : IsCollection(u) || IsComplex(u) || u.Namespace?.StartsWith("System.Text") == true ? typeof(string) : u;
            ...
            resolver.Modifiers.Add(typeInfo =>
            {
                ...
                foreach (JsonPropertyInfo prop in typeInfo.Properties)
                {
                    ...
                    if (ns.StartsWith("System.Text", StringComparison.Ordinal) || t.FullName!.StartsWith("System.ReadOnlySpan", StringComparison.Ordinal))
                    {
                        toRemove.Add(prop);
                    }
                }
            ...
            foreach (T item in a_rows)
            {
                ...
                foreach (PropertyInfo p in props)
                {
                    ...
                    {
                        // Newtonsoft JToken / extension-data friendly handling
                        if (val is JToken jt)
                        {
                            object cell = JTokenToNet(jt);
                            dr[p.Name] = cell ?? DBNull.Value;
                        }
                        else if (val is IDictionary<string, JToken> ext)
                        {
                            dr[p.Name] = SerializeExtensionData(ext);
                        }
                        else if (val is System.Text.Encoding enc)
                        {
        ...
        // ----------------- Create Table -----------------
        private static string CreateSqlTable(DataTable a_dt, string a_schema = "dbo")
        {
            ...
            return $@"IF OBJECT_ID('{fullName}','U') IS NOT NULL DROP TABLE {fullName};CREATE TABLE {fullName} ({string.Join(",\n    ", cols)});";
        }
        ...
        private static string SafeSerializeToString(object a_value, JsonSerializerOptions a_jsonOpts)
        {
            ...
            try
            {
                return System.Text.Json.JsonSerializer.Serialize(a_value, a_jsonOpts);
            }
            catch
            {
                return a_value.ToString() ?? string.Empty;
        ...
        // ----------------- JToken / ExtensionData helpers -----------------

        /// <summary>
        /// Converts a JToken into a cell-friendly value.
        /// JValue → underlying value; JObject/JArray → compact JSON string; Null → null.
        /// </summary>
        private static object JTokenToNet(JToken a_jt)
        {
            if (a_jt == null || a_jt.Type == JTokenType.Null) return null;

            if (a_jt is JValue v) return v.Value;

            return a_jt.ToString(Formatting.None);
        }

        /// <summary>
        /// Serializes ExtensionData (IDictionary&lt;string, JToken&gt;) to JSON after
        /// converting each token to a CLR value.
        /// </summary>
        private static string SerializeExtensionData(IDictionary<string, JToken> a_extra)
        {
            Dictionary<string, object?> flat = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase);
            foreach (KeyValuePair<string, JToken> kv in a_extra)
            {
                object? val = JTokenToNet(kv.Value);
        ...
        //Overload and helpers for dynamic JArray

        public static async Task DumpAsync(JArray a_rows, string a_connStr, string a_tableName, string a_schema)
        {            
            HashSet<string> cols = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (JToken token in a_rows)
            {
            ...
            DataTable dt = new DataTable(a_tableName);
            foreach (string c in cols) dt.Columns.Add(c, typeof(string));
            ...
            foreach (JToken token in a_rows)
            {
                ...
                if (token is JObject o)
                {
                    ...
                    {
                        if (o.TryGetValue(c, out JToken? val) && val != null && val.Type != JTokenType.Null)
                            dr[c] = val is JValue v ? (v.Value ?? DBNull.Value) : val.ToString(Formatting.None);
                        else
                            dr[c] = DBNull.Value;
                    }
                }
            ...
            string full = $"[{a_schema}].[{a_tableName}]";
            string ddl = $@"IF OBJECT_ID('{full}','U') IS NOT NULL DROP TABLE {full};
CREATE TABLE {full} ({string.Join(", ", cols.Select(c => $"[{c}] NVARCHAR(MAX) NULL"))});";

            await using SqlConnection conn = new SqlConnection(a_connStr);
            await conn.OpenAsync();

            await using (SqlCommand cmd = new SqlCommand(ddl, conn))
                await cmd.ExecuteNonQueryAsync();

            using SqlBulkCopy bulk = new SqlBulkCopy(conn) { DestinationTableName = full, BulkCopyTimeout = 0 };
