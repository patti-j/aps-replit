	...
	internal class Instance
	{
		...
		public static async Task<byte[]> CallHandshakeAPI(string publicKey, string url)
		{
			...
			using (HttpClient client = new HttpClient(httpClientHandler))
			{
				...
				catch (Exception ex)
				{
                    ...
                    throw new Exception($"An error occurred in CallHandshakeAPI: {ex.Message}");
                }
			}
		}
		...
		public static async Task<List<InstanceWithUser>> CallServerManagerToGetInstance(string ServerManagerURL, string UserName, string Password)
		{
			...
			using (HttpClient client = new HttpClient(httpClientHandler))
			{
				...
				{
					// Handle any exceptions that occur during the API call
					throw new Exception($"An error occurred in CallServerManagerToGetInstance: {ex.Message}");
				}
			}
		}
		/// <summary>
			...
			using (Stream stream = await response.Content.ReadAsStreamAsync())
			{
				...
				if (response.IsSuccessStatusCode)
				{
					if (stream == null)
					{
                        ...
                        //};
                        throw new Exception($"An error occurred: Response stream from instance is null");
                    }
					...
					if (!stream.CanRead)
					{
                        ...
                        throw new Exception($"An error occurred: Cannot read from stream from Instance");
                    }
		...
		public static async Task<UserLoginResponse> PostLogin(BasicLoginRequest request, string url)
		{
			...
			using (HttpClient client = new HttpClient(httpClientHandler))
			{
				...
				catch (Exception ex)
				{
                    ...
                    throw new Exception($"An error occurred in PostLogin: {ex.Message}");
				}
			}
		}
		...
		public static async Task<GetScenarioResponse> PostGetScenarios(GetScenarioRequest request, string sessionToken, string url)
		{
			...
			using (HttpClient client = new HttpClient(httpClientHandler))
			{
				...
				catch (Exception ex)
				{
					// Handle any exceptions that occur during the API call
					Console.WriteLine($"An error occurred: {ex.Message}");
                    throw new Exception($"An error occurred in PostGetScenarios: {ex.Message}");
				}
			}
		}
		...
		public static async Task<ImportResponse> PostRequestImport(ImportRequest request, string sessionToken, string url)
		{
			...
			using (HttpClient client = new HttpClient(httpClientHandler))
			{
				...
				catch (Exception ex)
				{
					// Handle any exceptions that occur during the API call
					Console.WriteLine($"An error occurred: {ex.Message}");
                    throw new Exception($"An error occurred in PostRequestImport: {ex.Message}");
                    return null;
				}
			}
		}
