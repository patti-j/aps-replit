...
public class DbSchemaExtractor
{
    public static void UploadSchemaToDb(SqlConnection a_connection, DBIntegrationDTO a_stagingDbUploadData)
        ...
        public static DBIntegrationDTO RetrieveDBSchema(SqlConnection a_connection) //TODO: figure out exception handling
        {
            a_connection.Open();
        ...
        private static void RetrieveTableOrViewDetails(SqlConnection a_connection, DataRow a_table, DBIntegrationDTO a_dbIntegrationDto)
        {
            ...
            else if ((string)a_table["TABLE_TYPE"] == "BASE TABLE")
            {
                ...
                foreach (var column in columns) //this isnt a great way to get the create script, ideally we would call an sp or something but apparently thats very hard to do with tables
                {
                    ...
                    {
                        if (column.Item2.Equals("nvarchar", StringComparison.OrdinalIgnoreCase) || column.Item2.Equals("varchar", StringComparison.OrdinalIgnoreCase))
                        {
                            sb.Append($"{column.Item1} {column.Item2}(128),\n");//Don't forget to add length for varchar types, otherwise the create statement will fail when we try to upload it to a new db. This is a band-aid for that issue, ideally we would pull the length from the db as well but this is better than nothing for now
                        }
                        else
                        {
