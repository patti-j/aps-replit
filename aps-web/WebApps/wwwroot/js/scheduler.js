import SchedulerManager from "./bryntum-modules/scheduler-manager.js";
export async function createSchedulerManager() {
    // This function will generate a JS file that injects styles into the page dynamically (if not already added)
    ...
    // JSON object containing the CSS variable name (without prefix) and its value
    const strokeMap = {
        ...
        indianred: '#CD5C5C', indigo: '#4B0082', ivory: '#FFFFFF', khaki: '#F0E68C', lavender: '#E6E6FA',
        ...
        tan: '#D2B48C', teal: '#008080', thistle: '#D8BFD8', tomato: '#FF6347', transparent: 'transparent',
        turquoise: '#40E0D0', violet: '#EE82EE', wheat: '#F5DEB3', white: '#FFFFFF', whitesmoke: '#F5F5F5',
        yellow: '#FFFF00', yellowgreen: '#9ACD32'
    };

    // Generate CSS content using the strokeMap
    const css = `:root {
        ${Object.entries(strokeMap).map(([name, value]) => `  --${name}-stroke: ${value};\n  --${name}-color: ${value};`).join('\n')}
    }

    ${Object.keys(strokeMap).map(name => `
        .color-${name} {
          stroke: var(--${name}-stroke) !important;
          color: var(--${name}-color) !important;
          background-color: ${strokeMap[name]}59 !important;
        }

        .child-color-${name} [data-task-feature] {
          stroke: var(--${name}-stroke) !important;
          color: var(--${name}-color) !important;
          background-color: ${strokeMap[name]}59 !important;
    }`).join('\n')}`;
