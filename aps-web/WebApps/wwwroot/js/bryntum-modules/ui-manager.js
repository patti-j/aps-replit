...
class UIManager {
    ...
    async highlightEvents(value) {
        ...
        if (scheduler && scheduler.eventStore) {
            let range = SchedulerUtils.updateEventHighlighting(scheduler, new RegExp(escapedValue, 'i'));
            console.log(range)
            SchedulerUtils.updateSchedulerUI(scheduler, 'b-highlighting', escapedValue.length > 0);
            if (escapedValue !== "")
            {
                await this.schedulerManager.schedulerZoomManager.zoomToTimeSpan({startDate: range.earliestStartDate, endDate: range.latestEndDate})
            }
        } else {
