...
export default class SchedulerEventUtils {
    /**
     * @return {{earliestStartDate: Date, latestEndDate: Date}}
     * */
    static updateEventHighlighting(scheduler, regex) {
        var earliestStartDate = new Date(8640000000000000)
        var latestEndDate = new Date(-8640000000000000)
        scheduler.eventStore.forEach(task => {
            ...
            if (regex && regex.test(task.name)) {
                ...
                if (task.startDate < earliestStartDate) {
                    earliestStartDate = task.startDate;
                }
                if (task.endDate > latestEndDate) {
        ...
        return {earliestStartDate: earliestStartDate, latestEndDate: latestEndDate};
    }
