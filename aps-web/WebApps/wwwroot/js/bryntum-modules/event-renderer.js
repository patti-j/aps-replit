class EventRenderer {
    ...
    getSegmentHTML(segment, index, segments, eventRecord) {
        ...
        return `<div class="event-section" style="${style}">
                ${customHTML
                ? segment.customCode
                : `<p>${this.schedulerManager.templatingEngine.generateSegment(segment.segmentName, eventRecord)}</p>` 
            }
            </div>`;
    }
    ...
    render = ({ eventRecord, resourceRecord, renderData }) => {
        ...
        const encodeHtml = segments.length > 0
            ? `<section class="b-sch-event-container">${segments.map((segment, index) => this.getSegmentHTML(segment, index, segments, eventRecord)).join('')}</section>`
            : "";
