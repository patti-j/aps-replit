...
[Obsolete("This list is obsolete. Use CustomSortedDictionary instead")]
public abstract class CustomSortedList<ValueT> where ValueT : IPTSerializable
{
    ...
    /// </summary>
    protected CustomSortedList(IKeyObjectComparer<ValueT> a_comparer)
    {
        m_sortedList = new List<ValueT>();
        m_comparer = a_comparer;
    }

    public CustomSortedList(IReader a_reader, IKeyObjectComparer<ValueT> a_comparer, BaseIdGenerator a_idGenerator = null)
        : this(a_comparer)
    {
    ...
    private const int UNIQUE_ID = 737;
    ...
    /// </summary>
    private readonly Dictionary<object, ValueT> m_table = new ();
    ...
    private readonly IKeyObjectComparer<ValueT> m_comparer;
    ...
    {
        object key = m_comparer.GetKey(a_element);
    ...
    public bool RemoveByKey(object a_key)
    {
        ValueT v = GetValue(a_key);
        if (v != null)
        {
    ...
    public ValueT GetValue(object a_key)
    {
    ...
    /// <returns></returns>
    public ValueT Find(object a_key)
    {
    ...
    /// </param>
    public bool TryGetValue(object a_key, out ValueT o_value)
    {
    ...
    /// <returns></returns>
    public bool ContainsKey(object a_key)
    {
...
[Obsolete("The boxing to object is slow. Use a typed version instead such as IBaseIdObjectComparer<T>")]
public interface IKeyObjectComparer<T> : IComparer<T>
{
    object GetKey(T a_element);
}