...
internal class TransferOrderDemandProfile : WarehouseDemandProfile
{
    private readonly TransferOrderDistribution m_tod;

    internal TransferOrderDemandProfile(TransferOrderDistribution a_tod) 
        : base(a_tod.Item, true, a_tod.FromWarehouse, new TransfersOrderDemandSource(a_tod))
    {
        m_tod = a_tod;
        AllowPartialSupply = a_tod.AllowPartialAllocations;
        AllowMultiStorageAreaSupply = true; //Always pull from multiple. //TODO: We could add a field on SO for this.
        m_materialAllocation = a_tod.MaterialAllocation;
        RemainingQty = a_tod.QtyOpenToShip;
    }

    internal override void GenerateProfile(ScenarioDetail a_sd)
    {
        QtyDemandNode qtyNode = new (a_sd.SimClock, m_tod, RemainingQty);
        AddToEnd(qtyNode);
    }

    internal override void UpdateActualAvailableTicks(long a_matlAvailTicks)
    {
        m_tod.ActualAvailableTicks = a_matlAvailTicks;
    }
    ...
    internal override bool IsSupplySourceEligible(QtySupplyNode a_node, ScenarioDetail a_sd)
    {
        ...
        //Check lot tracking:
        if (!m_tod.IsLotElig(a_node.SupplySource.SupplyLot, a_sd.UsedAsEligibleLotsLotCodes))
        {
            return false;
        }
